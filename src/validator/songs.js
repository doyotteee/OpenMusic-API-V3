...
const SongPayloadSchema = Joi.object({
  ...
  year: Joi.number().integer().min(1900).max(2100).required(),  performer: Joi.string().pattern(/^(?!\s*$).+/).required().messages({
    'string.pattern.base': 'Performer cannot be empty or only whitespace',
  }),
  genre: Joi.string().required(),  duration: Joi.alternatives().try(
